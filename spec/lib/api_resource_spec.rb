describe 'API Resource' do
  ...
  describe '.send_request' do
    ...
    it 'should not raise error if required param is present' do
      ...
      expect(Eligible::APIResource.send_request(:get, url, params, api_key: api_key, required_params: [:reference_id])).to eq 'success'
    end

    it 'should raise error if required param is not present' do
      expect { Eligible::APIResource.send_request(:get, url, params, api_key: api_key, required_params: [:reference_id]) }.to raise_error(ArgumentError)
    end
  end
end
