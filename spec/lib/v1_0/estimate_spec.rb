describe 'Eligible::Estimate' do
  let(:params) { { rest_api_version: '1.0' } }
  let(:api_key) { 'xyz' }
  let(:response) { { success: true } }
  before(:each) do
    allow(Eligible::Util).to receive(:convert_to_eligible_object).with(response, api_key).and_return('success')
    allow(Eligible::Estimate).to receive(:endpoint_name).and_return('estimates')
  end
  ...
  describe '.retrieve' do
    it 'should call Eligible.request with proper url' do
      ...
      expect(Eligible::Estimate.retrieve(params[:id], api_key: api_key)).to eq 'success'
    end

    it 'should raise error if estimate id is not present' do
      expect { Eligible::Estimate.retrieve(nil, api_key: api_key) }.to raise_error(ArgumentError)
    end
  end
  ...
  describe '.create' do
    it 'should post to Eligible.request with proper url' do
      ...
      expect(Eligible::Estimate.create(params, api_key: api_key)).to eq 'success'
    end
  end
  ...
  describe '.list' do
    it 'should call Eligible.request with proper url' do
      ...
      expect(Eligible::Estimate.list(params, api_key: api_key)).to eq 'success'
    end
  end
  ...
  describe '.update' do
    it 'should call Eligible.request with proper url' do
      ...
      expect(Eligible::Estimate.update(params, api_key: api_key)).to eq 'success'
    end

    it 'should raise error if estimate id is not present' do
      expect { Eligible::Estimate.update(params, api_key: api_key) }.to raise_error(ArgumentError)
    end
  end
end
